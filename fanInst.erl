...
create(Host, FanTyp_Pid, PipeInst_Pid, RealWorldCmdFn) -> {ok, spawn(?MODULE, init, [Host, FanTyp_Pid, PipeInst_Pid, RealWorldCmdFn])}.

init(Host, FanTyp_Pid, PipeInst_Pid, RealWorldCmdFn) ->
  {ok, State} = apply(resource_type, get_initial_state, [FanTyp_Pid, self(), [PipeInst_Pid, RealWorldCmdFn]]),
  %  get_initial_state  (ResTyp_Pid,  ResInst_Pid, TypeOptions)
  survivor:entry({ fanInst_created, State }),
  loop(Host, State, FanTyp_Pid, PipeInst_Pid).
...
loop(Host, State, FanTyp_Pid, PipeInst_Pid) ->
  receive
    switchOn ->
      ...
      loop(Host, NewState, FanTyp_Pid, PipeInst_Pid);
    switchOff ->
