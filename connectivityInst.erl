...
%%%
%%% @end
%%% Created : 16. Dec 2018 13:13
%%%-------------------------------------------------------------------
-module(connectivityInst).
-author("Changizi").
...
init(Root_ConnectorPid, ResTyp_Pid) ->
  ...
  survivor:entry({ connectivityInst_created, State }),
...
loop(Root_ConnectorPid, State, ResTyp_Pid) ->
  receive
    ...
    {get_resource_circuit, ReplyFn} ->
      {ok, C} = connectivityType:get_resource_circuit(ResTyp_Pid, State),
      ReplyFn(C),
      loop(Root_ConnectorPid, State, ResTyp_Pid)
  end.
